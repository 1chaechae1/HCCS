...
$(function(){
    areaSelectMaker("#searchHopeArea1", "searchHopeArea2", "#searchHopeArea3");
});

var areaSelectMaker = function(s1, s2, s3){
    ...
    areaKeys1.forEach(function(Area1){
        $(s1).append("<option value="+Area1+">"+Area1+"</option>");
    });
    ...
    $(document).on("change", s1, function(){
        init(false, true);
        var Area1 = $(this).val();
        var keys = Object.keys(area[Area1]);
        keys.forEach(function(Area2){
            $(s2).append("<option value="+Area2+">"+Area2+"</option>");    
        });
    }).on("change", s2, function(){
        init();
        var Area1 = $(s1).val();
        var Area2 = $(this).val();
        var keys = Object.keys(area[Area1][Area2]);
        keys.forEach(function(Area3){
            $(s3).append("<option value="+area[Area1][Area2][Area3]+">"+area[Area1][Area2][Area3]+"</option>");    
        });
    });

    function init(first, second){
        first ? $(s1).empty().append("<option value=''>선택</option>") : "";
        second ? $(s2).empty().append("<option value=''>선택</option>") : "";
        $(s3).empty().append("<option value=''>선택</option>");
    }
}